...
data <-
  readRDS(
    "~/Box/snRNA_CellRanger_Wound_nonWound/objects/final_integrated_clustered1118.rds"
  ) |>
  subset(type == "wound")
...
cellchat_normal <-
  subsetData(cellchat_normal) # This step is necessary even if using the whole database
future::plan("multisession", workers = 4)


cellchat_normal <- identifyOverExpressedGenes(cellchat_normal, thresh.p = .1)
cellchat_normal <-
  identifyOverExpressedInteractions(cellchat_normal)

gc()

cellchat_normal <- computeCommunProb(cellchat_normal, trim = 0)

gc()

# Filter out the cell-cell communication if there are only few number of cells in certain cell groups
cellchat_normal <-
  filterCommunication(cellchat_normal, min.cells = 10)


cellchat_normal <-
  computeCommunProbPathway(cellchat_normal, thresh = .1)
...
svglite(paste0(fig_dir, file = "pathway_count.svg"),
        width = 20,
        height = 10)
...
fig_dir <- "C:/Users/ranoj/Desktop/Single_cell_output/cellchat/"
svglite(paste0(fig_dir, file = "pathway_weight.svg"),
        width = 20,
        height = 10)
#pdf(paste0(fig_dir, "count.pdf"), width=2.45, height=1.3)
#(p1|p2)/p3 +umap_theme
...
for (i in 1:length(pathways.show.all)) {
  ...
  gg <-
    netAnalysis_contribution(cellchat_normal, signaling = pathways.show.all[i])
  ggsave(
    filename = paste0(pathways.show.all[i], "_wound2_L-R_contribution.svg"),
    plot = gg,
    width = 3,
    height = 2,
    units = 'in',
...
ggsave(
  filename = paste0(pathways.show.all[i], "_L-R_contribution.pdf"),
  plot = gg,
  width = 3,
  height = 2,
  units = 'in',
