...
ggsave(
  "~/Box/snRNA_CellRanger_Wound_nonWound/targeted filtering/sctransform/ptprc/umap.png"
)
DimPlot(combined_sct, split.by = "injury")
ggsave(
  "~/Box/snRNA_CellRanger_Wound_nonWound/targeted filtering/sctransform/ptprc/umap-split.png"
)
...
combined_sct %>%
  FindAllMarkers(assay = "RNA",
                 only.pos = T) %>%
  write.csv(
    "~/Box/snRNA_CellRanger_Wound_nonWound/targeted filtering/sctransform/ptprc/top-markers.csv"
  )
...
for (i in combined_sct$seurat_clusters %>% unique()) {
  ...
  Tweedieverse(
    ...
    output = paste0(
      '~/Box/snRNA_CellRanger_Wound_nonWound/targeted filtering/sctransform/ptprc/tweedieverse/cluster-',
      i
    ),
