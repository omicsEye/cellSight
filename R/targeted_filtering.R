...
>>>>>>> 78f61999fa48799f25a5cd4685a720387caf33dc
...
=======
  ...
  Pdgfra_expression = GetAssayData(object = pbmc, assay = "RNA", slot = "data")["Ptprc",]
>>>>>>> 78f61999fa48799f25a5cd4685a720387caf33dc
...
=======
  write.table(pos_cells.markers, file=paste0("data_labelled_", samples,sample,".tsv"), quote=FALSE, sep='\t', col.names = NA)
  write.table(pos_cells.markers, file=paste0("data_top_2_labelled_", samples,sample,".tsv"), quote=FALSE, sep='\t', col.names = NA)
  head(Idents(pos_cells), 5)
  features <- c("CD68", "Adgre1", "Ptprc","Pdgfra", "Pdgfrb", "Col1a1","Krt14", "Krt10", "Krt5","Plin1", "Adipoq", "Pparg", "Fabp4","Ptprc","Pecam1", "CD34")
  ridge <- RidgePlot(pos_cells, features = features, ncol = 4)
  ggsave(paste0("analysis/figures/ridge", samples,sample,".pdf", sep="") ,plot=ridge, width = 7.2, height = 4, units = "in", dpi = 350)
  violin <- VlnPlot(pos_cells, features = features)
  feature_plot <- FeaturePlot(pos_cells, features = features)
