...
Adgre1_expression = GetAssayData(object = pbmc, assay = "RNA", slot = "data")["Adgre1",]

pos_ids = names(which(Adgre1_expression>0))

pos_cells_macro = subset(pbmc,cells=pos_ids)

print(intersect(pos_cells, pos_cells_macro))

pos_cells <- merge(x = pos_cells, y = pos_cells_macro)
...
pos_cells_nonwound_macro_pdgfrb <- pos_cells
